...
function animate() {
  rules.map(el => {
    ...
    prefixes.map(prefix => {
      el.element.style[prefix + "transform"] = "translate(" + translate + ", 0)";
      if (el.transition) el.element.style.[prefix + "transition"] = el.transition;
    })
  });
}
...
function returnBack() {
  rules.map(el => {
    el.element.style.transform = "translate(0, 0)";
  });
}
